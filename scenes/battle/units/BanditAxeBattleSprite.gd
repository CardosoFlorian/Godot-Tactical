class_name BanditAxeBattleSprite
extends Node2D
## Bandit (axe) battle sprite — same flipbook pattern as Aurora/Lycith/
## Martin/Kessa. Idle loops from assets/units/bandit_axe/idle/ (13 frames),
## attack plays once from assets/units/bandit_axe/attack/ (14 frames — one
## big overhead axe chop). Real idle+attack art replaced the original
## single-pose reference the same day it was integrated — see
## bandit_enemy_designs memory for why the scale/offset numbers below don't
## match that first, now-obsolete single-pose version. This enemy is a
## generic, reused "Bandit" archetype (see enemy_naming_convention memory)
## — no personal name anywhere, including in code: `character_id` is
## literally "bandit_axe".

const IDLE_FRAME_DIR := "res://assets/units/bandit_axe/idle/"
const IDLE_FRAME_COUNT := 13
const IDLE_FPS := 8.0

const ATTACK_FRAME_DIR := "res://assets/units/bandit_axe/attack/"
